...
<* IF DEST_K26 THEN *>
  ...
  SetModeName (pt.Pak,           'PAK',            'ПАКЕТ');
  SetModeName (pt.Trace,         'TRACE',          'ТРАССА');
  SetModeName (pt.Put,           'PUT',            'ПЕЧАТЬ');
  SetModeName (pt.IO,            'IO',             'ОБМЕН');
  SetModeName (pt.Time_Count,    'TIME',           'ВРЕМЯ');
  SetModeName (pt.Put_Addr,      'ADR',            'АДРЕС');
  SetModeName (pt.Dump,          'DUMP',           'ДАМП');
  SetModeName (pt.User,          'ПОЛЬЗ',          'USER');
  SetModeName (pt.OutMem,        'ВП',             'OUT');
  SetModeName (pt.WrProt,        'ПЗУ',            'WRPR');
  SetModeName (pt.ProgInt,       'ПР',             'PR');
  SetModeName (pt.Address,       'АДР',            'ADDR');
  SetModeName (pt.Write,         'З',              'WRITE');
  SetModeName (pt.WriteReg,      'З',              'WRITE');
  SetModeName (pt.Read,          'Ч',              'READ');
  SetModeName (pt.ReadReg,       'Ч',              'READ');
  SetModeName (pt.Call,          'ВЫЗОВ',          'CALL');
  SetModeName (pt.Ret,           'ВОЗВРАТ',        'RET');
  SetModeName (pt.If,            'ЕСЛИ',           'IF');
  SetModeName (pt.Line,          'С',              'LINE');
  SetModeName (pt.IO,            'ВВ',             'IO'  );
  SetModeName (pt.System,        'СИСТЕМА',        'SYSTEM');
  SetModeName (pt.Storage,       'ПАМЯТЬ',         'STORAGE');
  SetModeName (pt.StoragePrint,  'ПАМЯТЬ_ПЕЧАТЬ',  'STORAGE_PRINT');
  SetModeName (pt.CompCreated,   'ФАЗА',           'COMP' );
  SetModeName (pt.CompDestroyed, 'ФАЗАУД',         'COMPDEL');
  SetModeName (pt.Test,          'ТЕСТ',           'TEST');
  SetModeName (pt.ModelProc,     'ПРОЦ_ПЕЧАТЬ',    'PROC');
  SetModeName (pt.DirectStart,   'СТАРТ',          'START');
